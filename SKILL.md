...
7. For the ready route, read `validation.json`; for custom charts, run the chart-specific checks and keep their results. Then open the actual PNGs using the host's image viewer, check title/label overlap, clipping, source readability and truthful scale. Use explicit heading line advance and a separate title-to-description gap from the design system; verify the last title line does not crowd the description. Inspect the PDF when that is the requested acceptance format. Fix concrete failures before delivering. If visual tools are unavailable, state that the output is generated but visually unverified.
